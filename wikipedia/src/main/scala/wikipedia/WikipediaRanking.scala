...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((cnt, next) => {
      if (next.text.split(" ").contains(lang)) cnt + 1
      else cnt
    }, _ + _)
  }

  def occurrencesOfLang(lang: String, rdd: Iterable[WikipediaArticle]): Int = {
    rdd.aggregate(0)((cnt, next) => {
      if (next.text.split(" ").contains(lang)) cnt + 1
      else cnt
    }, _ + _)
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs
    ...
    .sortBy(x => x._2)(Ordering[Int].reverse)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues(_.size).sortBy(x => x._2, ascending = false).collect().toList
  }
